...
export const AccessibilityWidget: React.FC = () => {
  ...
  return (
    <>
      ...
      <button 
        onClick={toggleOpen}
        className="fixed bottom-6 left-6 z-[60] w-14 h-14 bg-latte-900 text-latte-50 rounded-full shadow-2xl flex items-center justify-center hover:scale-105 transition-transform duration-300 focus:outline-none focus:ring-4 focus:ring-latte-300 border-2 border-latte-100"
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-latte-900/40 backdrop-blur-sm z-50"
          onClick={() => setIsOpen(false)}
        />
      )}

      {/* Drawer */}
      <div className={`fixed inset-y-0 left-0 z-[70] w-full md:w-96 bg-latte-50 shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col border-r border-latte-200`}>
        
        {/* Header */}
        <div className="p-6 border-b border-latte-200 bg-white flex items-center justify-between shrink-0">
          <h2 className="font-display font-bold text-xl text-latte-900">
            {view === 'settings' ? 'Accessibility Tools' : 'Accessibility Statement'}
          </h2>
          <button 
            onClick={() => setIsOpen(false)}
            className="w-10 h-10 rounded-full hover:bg-latte-100 flex items-center justify-center text-latte-500 transition-colors"
          >
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 bg-latte-50">
          {view === 'settings' ? (
            <div className="space-y-4">
              <p className="text-sm text-latte-800 font-sans mb-4">
                Customize your viewing experience with our accessibility tools.
              </p>

              {/* Text Size Control */}
              <div className="bg-white p-4 rounded-xl border border-latte-200">
                 <div className="flex items-center gap-2 mb-3 text-latte-900 font-bold">
                 ...
                 <div className="flex gap-2">
                    <button 
                       ...
                       className={`flex-1 py-2 rounded-lg text-sm font-bold border transition-colors ${settings.textSize === 'normal' ? 'bg-latte-900 text-white border-latte-900' : 'bg-latte-50 text-latte-800 border-latte-200 hover:bg-latte-100'}`}
                    >
                      Aa
                    </button>
                    <button 
                       onClick={() => setTextSize('large')}
                       className={`flex-1 py-2 rounded-lg text-base font-bold border transition-colors ${settings.textSize === 'large' ? 'bg-latte-900 text-white border-latte-900' : 'bg-latte-50 text-latte-800 border-latte-200 hover:bg-latte-100'}`}
                    >
                      Aa
                    </button>
                    <button 
                       onClick={() => setTextSize('xl')}
                       className={`flex-1 py-2 rounded-lg text-lg font-bold border transition-colors ${settings.textSize === 'xl' ? 'bg-latte-900 text-white border-latte-900' : 'bg-latte-50 text-latte-800 border-latte-200 hover:bg-latte-100'}`}
                    >
                      Aa
                    </button>
                 </div>
              ...
              <div className="mt-8 pt-6 border-t border-latte-200">
                <button 
                  ...
                  className="w-full py-3 px-4 bg-latte-100 hover:bg-latte-200 text-latte-900 rounded-xl font-bold transition-colors text-sm"
                >
            ...
            <div className="prose prose-sm prose-latte max-w-none text-latte-800 animate-slide-in-left">
              <button 
                onClick={() => setView('settings')}
                className="flex items-center gap-2 text-latte-900 font-bold mb-6 hover:text-latte-500 transition-colors"
              >
                <ChevronLeftIcon className="w-5 h-5" />
                Back to Settings
              </button>
              
              <h3 className="font-display font-bold text-lg text-latte-900 mb-2">Our Commitment to Accessibility</h3>
              ...
              <h3 className="font-display font-bold text-lg text-latte-900 mb-2">New Features</h3>
              ...
              <h3 className="font-display font-bold text-lg text-latte-900 mb-2">Compliance Status</h3>
              ...
              <h3 className="font-display font-bold text-lg text-latte-900 mb-2">Contact Us</h3>
        ...
        {/* Footer */}
        {view === 'settings' && (
          <div className="p-6 border-t border-latte-200 bg-white shrink-0">
            <button 
              onClick={resetSettings}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 border-2 border-latte-900 text-latte-900 hover:bg-latte-50 rounded-xl font-bold transition-colors"
            >
...
const ToggleButton = ({ label, icon, active, onClick }: { label: string, icon: React.ReactNode, active: boolean, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`w-full p-4 rounded-xl border flex items-center gap-4 transition-all ${active ? 'bg-latte-900 text-white border-latte-900 shadow-md' : 'bg-white text-latte-900 border-latte-200 hover:border-latte-400'}`}
  >
    {icon}
    <span className="font-bold flex-1 text-left">{label}</span>
    <div className={`w-10 h-6 rounded-full p-1 transition-colors ${active ? 'bg-latte-400' : 'bg-latte-200'}`}>

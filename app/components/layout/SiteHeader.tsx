...
export default function SiteHeader({
  ...
  const forceSolidBg = forceSolidRoutes.some((route) =>
  ...
  const instagramHref =
    instagramUrl || "https://instagram.com/thenotebookcafellc";
  ...
  const logoSrc = useLightText
    ? "/tnc-navbar-logo-dark-v1.png"
    : "/tnc-navbar-logo-light-v1.png";
        ...
        window.requestAnimationFrame(() => {
          ...
          if (atTop) {
            ...
            const directionChanged =
              (delta > 0 && scrollDeltaAcc.current < 0) ||
              (delta < 0 && scrollDeltaAcc.current > 0);
            ...
            if (
              scrollDeltaAcc.current > toggleThresholdPx &&
              currentScrollY > hideAfterPx
            ) {
              setIsVisible(false);
  ...
  useEffect(() => {
    const updateContrast = () => {
      ...
      const isHideImages =
        document.documentElement.classList.contains("acc-hide-images");
  ...
  return (
    <>
        ...
        >
          <div className="grid grid-cols-[auto_1fr_auto] items-center transition-all duration-500 py-4">
            ...
            <div className="hidden md:flex items-center justify-end gap-4">
              ...
              >
                <ShoppingBag
                  size={18}
                  className={
                    useLightText ? "text-coffee-50" : "text-coffee-900/80"
                  }
                />
                {cartCount > 0 && (
                  <span
              ...
              >
                ...
                {/* Follow Button */}
                <span
                  className={`${navLinkBase} !text-2xs px-3 py-1.5 border rounded transition-all ${useLightText ? "border-coffee-50/55 text-coffee-50 bg-white/5 hover:bg-white/8 hover:border-coffee-50/80 hover:text-coffee-50/90" : "border-coffee-900/50 text-coffee-900 bg-coffee-900/5 hover:bg-coffee-900/8 hover:border-coffee-900/70 hover:text-coffee-900/90"}`}
                >
                  Follow
                </span>
              </a>
            ...
            <div className="md:hidden flex items-center justify-end gap-6">
              ...
              >
                <span
                  className={`hamburger-icon ${useLightText ? "hamburger-light" : ""}`}
                >
                  <span
      ...
      {/* Cinematic Mobile Menu - Triple Layer */}
      {/* Layer 1: Dim Overlay (Full screen, click to close) */}
      <div
        className={`menu-dim-overlay md:hidden ${isOpen ? "open" : ""}`}
        onClick={() => setIsOpen(false)}
      />

      {/* Layer 2: Tan Panel (Slides from right) */}
      <div
        className={`menu-layer-backdrop md:hidden ${isOpen ? "open" : ""}`}
      />

      {/* Layer 3: Main Drawer (Slides from right) */}
      <div
        className={`menu-drawer-cinematic md:hidden ${isOpen ? "open" : ""}`}
      >
          ...
          {[
            ...
            <div key={link.href} className="menu-link-wrapper">
              <div
                ...
                style={{
                  transitionDelay: isOpen ? `${0.15 + index * 0.05}s` : "0s",
                }}
              >
        ...
        <div className="menu-drawer-footer">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="menu-drawer-footer-social"
          >
          ...
          <div className="menu-drawer-footer-text">Riverside, CA</div>
        </div>
      </div>
    </>
  );
